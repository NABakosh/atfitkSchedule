...
export interface ParsedLesson {
	...
	groupName: string
	course: string
	discipline: string // Теперь это поле будет содержать Дисциплину + Преподавателя
	teacher: string // Оставляем пустым или используем для специфических нужд, но в данном случае лучше оставить как пустую строку
	period?: string
}
 ...
 */
function extractFullLessonText(lessonData: string): {
	...
	const periodMatch = fullDisciplineText.match(
		/(\s*До\s+\d{2}\.\d{2}\.\d{2}г\.\s*)$/i
	)
	if (periodMatch) {
...
export function parseSchedule(rawData: RawScheduleEntry[]): ParsedLesson[] {
	const parsedData: ParsedLesson[] = []
	...
	rawData.forEach(row => {
		...
		Object.keys(row).forEach(key => {
			...
			rawLessons.forEach((lessonData, index) => {
				...
				const finalPeriodText =
					finalDisciplineText + (period ? ` (${period})` : '')
